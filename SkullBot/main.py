...
'''слово List, выполняет команду на выдачу файла .csv со списком доступных коинов'''

@bot.message_handler(func= lambda message: message.text == 'List')
def money_list(message):
    bot.send_message(message.chat.id, 'Скачать список доступных коинов')
    get_coin_list(api)
    with open('coin_list.csv', 'rb') as file:
        bot.send_document(message.chat.id, document=file)

'''Слово Search и запрашивает текст для поиска валют, который автоматически будет отловлен и будет
    выполнена функция pair_check'''

@bot.message_handler(func= lambda message: message.text == 'Search')
def pair_search(message):
    bot.send_message(message.chat.id, 'Введи через пробел валюту 1, валюту 2, количество валюты 1 \nК примеру: btc usd 100')
    bot.register_next_step_handler(message, pair_check)


'''функция получает данные , фильтрует их и совершает запрос на их основании '''
def pair_check(message):
    try:
        ...
        bot.send_message(message.chat.id, 'Некорректный ввод. Посмотри шаблон запроса')
        start(message)
    except Exception as e:
        print(e)
        bot.send_message(message.chat.id, 'Что то пошло не так, давай заново')
        start(message)
